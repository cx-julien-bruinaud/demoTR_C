...
request_id = LocalProxy(lambda: g.get('request_id', None) if has_request_context() else None)

def get_request_context():
    """Get the current request context (returns g object for Flask 2.x compatibility)"""
    if has_request_context():
        return g
    return None
